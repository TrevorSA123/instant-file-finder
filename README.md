...
These are the practical checks I use to make sure the main workflows still behave properly.

1. Start the app with no previous settings.

   Confirm that no indexing starts automatically and the status bar shows:

   ```text
   Live scan (no index)
   ```

2. Search for something and confirm that results appear incrementally.

3. Turn on **Enable indexing** in Preferences.

   Confirm that the fixed NTFS drives begin indexing in the background and that the status bar shows progress.

4. Once indexing finishes, confirm that the status bar shows the actual method used, such as Raw MFT or NTFS USN.
...
5. Search for:
   ...
   *.exe
   ...
   Confirm that wildcard matching works in both live and indexed modes.
...
6. Search for:
   ...
   Confirm that only `.log` files appear.
...
7. Search for:
   ...
   Confirm that only paths containing `users` appear.
...
8. Search for:
   ...
   After allowing some visible rows to be enriched, confirm that large files appear.

9. Select a result and use **Open Selected**.

10. Select a result and use **Open Containing Folder**.

11. Select a result and use **Copy Selected Path**.

    Paste it somewhere and confirm the path is correct.

12. Right-click a result and confirm the context menu actions are available.

13. Use **Delete** on a test file and confirm that it goes to the Recycle Bin, not permanent deletion.

14. Run a search and then use **Copy Results**.

    Paste into a text editor and confirm the output is tab-separated and includes a header, query, and timestamp.

15. Start indexing a large drive, then use **Index > Cancel Indexing**.

    Confirm that indexing stops and partial results remain searchable.

16. Use **Index > Rebuild Index**.

    Confirm that the drive is scanned again from scratch.

17. Run the app without Administrator rights, enable Raw MFT in Preferences, and confirm that it falls back to NTFS USN automatically if Raw MFT cannot run.

18. Accept the **Run as Administrator** relaunch offer and confirm Raw MFT works once elevated.

19. Enable a removable or non-NTFS drive in **Index Drives...**.

    Confirm that it uses recursive scanning.

20. Search for something broad enough to exceed the maximum displayed result count.
